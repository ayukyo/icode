(function () {
  ...
  var translations = {
    "zh-CN": {
      ...
      interactiveHelp: "适合需要逐步沟通和确认的需求。", autonomousHelp: "记录自动处理意图；本版本需等待自动执行器激活，期间仍按会话模式运行。", cancel: "取消", submitTicket: "提交工单", submitting: "正在提交…", created: "工单已创建", noSteps: "暂无", unknown: "未知",
    ...
    "en-US": {
      ...
      interactiveHelp: "Best when the request needs ongoing clarification and confirmation.", autonomousHelp: "Records autonomous intent. This release stays interactive until the autonomous runner activates it.", cancel: "Cancel", submitTicket: "Submit ticket", submitting: "Submitting…", created: "Ticket created", noSteps: "None", unknown: "Unknown",
  ...
  var state = { locale: "zh-CN", projects: [], tickets: [], selectedTicketId: null, selectedTicket: null, statusKey: "connecting", statusDetail: "" };
  ...
  function renderDetail(ticket) {
    ...
    byId("detail-raw-status").textContent = ticket.status; byId("detail-steps").textContent = (ticket.completed_steps || []).join(", ") || text("noSteps"); byId("detail-locale").textContent = ticket.locale; renderTickets(state.tickets);
  }
  function selectTicket(ticketId) {
    fetch("/api/v1/tickets/" + encodeURIComponent(ticketId), { credentials: "same-origin" }).then(function (response) { if (!response.ok) { throw new Error("HTTP " + response.status); } return response.json(); }).then(function (payload) { renderDetail(payload.ticket); }).catch(function () { setStatus("unavailable"); });
  }
  ...
  function bootstrap() {
    ...
    fetch("/api/v1/bootstrap", { credentials: "same-origin" }).then(function (response) { if (!response.ok) { throw new Error("HTTP " + response.status); } return response.json(); }).then(function (payload) { state.projects = payload.projects || []; state.tickets = payload.tickets || []; if (state.projects[0]) { byId("project-name").textContent = state.projects[0].name; } renderTickets(state.tickets); setStatus("connected"); }).catch(function () { setStatus("unavailable"); });
  }
