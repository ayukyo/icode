(function () {
  ...
  var translations = {
    "zh-CN": {
      ...
      autonomyEyebrow: "自动处理", autonomyTitle: "自主运行", pauseSemantics: "仅在当前契约步骤完成后暂停；正在进行的模型调用不会被中途截断。", runtimeCapability: "运行能力", configuredLimits: "配置限制", lastStep: "最近步骤", capabilityEnabled: "已启用（仅服务端配置）", capabilityDisabled: "未启用；需由管理员启动工作台能力", limitsFormat: "最多 {turns} 回合 · token 预算 {tokens} · 隔离 {isolation}", isolationEnforced: "已强制", isolationApplicationOnly: "仅应用层限制", isolationNotConfigured: "未配置", startAutonomy: "启动自动处理", pauseAutonomy: "在步骤边界暂停", resumeAutonomy: "继续自动处理", cancelAutonomy: "取消自动处理", takeoverAutonomy: "接管为会话模式", intentAccepted: "操作已接受", pending: "等待启动", starting: "正在启动", running: "正在自动处理", pause_requested: "已请求暂停", paused: "已暂停", cancel_requested: "已请求取消", cancelled: "已取消", succeeded: "自动处理已完成", failed: "自动处理失败", blocked: "已被门禁阻断", interrupted: "已中断",
  ...
  function renderDetail(ticket) {
    ...
    byId("detail-raw-status").textContent = ticket.status; byId("detail-steps").textContent = (ticket.completed_steps || []).join(", ") || text("noSteps"); byId("detail-locale").textContent = ticket.locale; renderAutonomy(ticket); renderTickets(state.tickets); scheduleActivePoll(ticket);
  }
