...
## 2026-09-26 UTC 上游复核与阶段边界刷新

- **Codex：**固定源码快照 [`1bf73324cadc72a53ed467edc7d3fd2b145a6166`](https://github.com/openai/codex/commit/1bf73324cadc72a53ed467edc7d3fd2b145a6166)。[Linux sandbox 说明](https://github.com/openai/codex/blob/1bf73324cadc72a53ed467edc7d3fd2b145a6166/codex-rs/linux-sandbox/README.md)将代码根下 `.git` 与解析后的 `gitdir:` 作为只读根；[Git helper](https://github.com/openai/codex/blob/1bf73324cadc72a53ed467edc7d3fd2b145a6166/codex-rs/git-utils/src/info.rs)使用 `GIT_OPTIONAL_LOCKS=0`、hook/fsmonitor 约束和超时。该机制与 ICODE 的 OS 只读 metadata grant 方向一致，但不构成独立 broker 或跨平台证据。
- **Qwen Code：**固定源码快照 [`9e60263fdeff8cb5bf5fc49287a2d20cef0dbe2e`](https://github.com/QwenLM/qwen-code/commit/9e60263fdeff8cb5bf5fc49287a2d20cef0dbe2e)。其[沙箱文档](https://github.com/QwenLM/qwen-code/blob/9e60263fdeff8cb5bf5fc49287a2d20cef0dbe2e/docs/users/features/sandbox.md)支持策略失配 fail-closed、项目设置不可降低操作者策略；文档也提醒 network closed 不会自动隐藏宿主文件或 socket。采纳边界原则，不复制容器配置。
- **Gemini CLI：**固定源码快照 [`2fe7c2d3f065dc40ad573d50b2091116f8a4aa18`](https://github.com/google-gemini/gemini-cli/commit/2fe7c2d3f065dc40ad573d50b2091116f8a4aa18)，[Git 环境处理](https://github.com/google-gemini/gemini-cli/blob/2fe7c2d3f065dc40ad573d50b2091116f8a4aa18/packages/core/src/utils/gitUtils.ts)清理 Git 配置环境；但这并非具备 OS 元数据只读证明的状态 broker。其后修正提交 [`562f0361fe63952fcf2db793e3e9fc0ae69ec506`](https://github.com/google-gemini/gemini-cli/commit/562f0361fe63952fcf2db793e3e9fc0ae69ec506) 删除 `diff.external=''`，与 ICODE 已移除该无关参数的修正相符。
- **采纳 / 暂缓：**采纳可信工作区身份、OS 强制元数据只读、固定参数/环境、helper 控制和失配拒绝；不把过滤 `GIT_*` 当沙箱，不移植上游实现或许可证不明代码。ICODE 当前仅有 Linux x86_64 已安装 wheel 原型，未接入 `ToolContext`；ARM64 原生 wheel、并发身份/配置漂移、timeout/output/child cleanup 压力测试、跨平台等价合同及真实工单调用链仍未闭合。继续保持 `git_broker_unavailable`，Git 自动工具端口关闭。观察日期：2026-09-26。

## 2026-09-26 Linux Git helper 执行面收窄

- **真实缺口与RED复现：**`execute_git_status()` 先读取 filter 配置，再启动 status；同一宿主并发者可在两步之间新增 `filter.<name>.clean`。真实临时 worktree 探针证明普通 Git 会执行其 `/usr/bin/python3` helper，并将标记输出写入 status 结果。单靠工作区只读不能阻止系统路径 helper 执行。
- **隔离修正：**Linux Landlock helper 增加可选第二策略层，仅允许精确的固定 payload ELF 与其 `PT_INTERP` 动态加载器执行；其余系统 helper（如 `/bin/sh`、Python）均被拒绝。Python 侧解析 ELF program-header 时限于受信系统二进制、表项数量/长度有界；helper 通过逐组件 `openat(O_NOFOLLOW)` 打开对象、核对 device/inode，并以同一已打开对象安装规则，路径或 inode 漂移时 payload 不启动。仅由内部 Git broker 传入该约束，通用 `run_command` 包装维持原行为。Linux 当前 ABI 不支持、ELF 不受支持、身份不匹配或后端失败均拒绝查询。
- **双架构前本机证据：**实际 Landlock kernel 回归中，固定 Python payload 及其 loader 可启动、其子进程 `/bin/sh` 被拒；伪造的 payload inode 在 helper 安装规则前被拒绝，payload 未启动；普通固定 Git status 仍能返回状态。注入的迟到 clean filter 无法执行，broker 不返回部分状态。以上仅是当前 x86_64 主机证据；ARM64 wheel/原生 CI 尚待验证。
- **额外兼容性修正：**本机 Git 2.34.1 在 `core.fsmonitor=false` 下会把 `false` 解释为 hook pathname（并尝试运行系统 `false`）。Git 官方文档明确指出 Git 2.35.1 及更早版本把 boolean `false` 当 pathname；通过本机 A/B 验证，固定空值 `core.fsmonitor=` 不再选择外部 helper。因此 broker 与 workspace 内部 Git 命令改为固定空覆盖。依据 [Git config 官方文档](https://git-scm.com/docs/git-config#Documentation/git-config.txt-corefsmonitor)。
- **干净安装 wheel 环境负例：**已安装包探针在一个恶意 `GIT_CONFIG_COUNT` clean-filter 可被普通 Git 执行的正向对照后，给真实 broker 子进程继承 `GIT_CONFIG_*`、`GIT_DIR`、`GIT_WORK_TREE` 和错误 `GIT_INDEX_FILE`，并验证状态仍绑定可信会话、helper marker 不出现。`scripts/run_native_wheel_ci.py` 本机完整构建/检查/隔离 venv 安装/wheel helper probe/Git broker probe 均通过。该验收仅为当前 x86_64；ARM64 CI 还未验证。
