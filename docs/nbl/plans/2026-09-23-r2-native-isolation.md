...
- 日期：2026-09-23
- 状态：main 持续实施中；尚未达到 R2.2 退出条件，自动模式仍阻断
...
- [macOS 双架构负例](https://github.com/ayukyo/icode/actions/runs/35960377550)确认：实验性 Seatbelt 策略下的后台孙进程可成功 `setsid` 脱离 broker 的进程组，同时 broker 仍返回组级 `cleanup_ok=True`；[后续双架构复测](https://github.com/ayukyo/icode/actions/runs/35960713928)又确认它在 broker 返回后延迟写出残留标记。Apple [XNU `setsid`/`setpgid` 实现](https://github.com/apple-oss-distributions/xnu/blob/main/bsd/kern/kern_prot.c)未显示可由当前 Seatbelt profile 拒绝这两个调用的检查点；这是源码与实测共同支持的当前机制缺口，不推断所有 macOS 隔离方案都不可行。`process_tree_cleanup` 关键合同在 macOS **未通过**，当前 `MacSeatbeltSandbox` 仍不提供生产 `wrap_policy`，不得启用自动模式。
- `icode doctor` 的诊断措辞同步收紧：展示选中后端自己声明的未覆盖能力，明确现有局部原生负向测试不等于执行完 10 项一致性评分；无论最小探测结果如何，策略级自动模式仍显示未就绪。
- 系统自带 `launchd` 独立作业曾是 macOS 清理候选：Apple [launchctl/launchd 文档](https://github.com/apple-oss-distributions/launchd/blob/main/man/launchctl.1)提供按唯一 label 管理作业，[其他开源 Agent 的现场问题](https://github.com/NousResearch/hermes-agent/issues/69098)提示仅 `setsid` 不一定能逃离 launchd 作业的更宽生命周期。但 [双架构 `submit/remove` 实测](https://github.com/ayukyo/icode/actions/runs/35961377590)和[双架构用户级 LaunchAgent `bootstrap/bootout` 实测](https://github.com/ayukyo/icode/actions/runs/35961720776)均显示：命令成功执行，脱组孙进程仍延迟写出残留。测试已按当前限制锁定已知失败且要求生产入口继续关闭；这两种无额外权限用法不能作为 R2.2 整树回收方案，未接入生产 broker。不能从这些负例推断所有 `launchd` 配置均不可行。
- Apple 新的 [`es_new_descendants_client`](https://developer.apple.com/documentation/endpointsecurity/es_new_descendants_client%28_%3A_%3A%29)可按调用者后代树接收进程事件，较轮询 PID/进程组更接近完整追踪；但官方文档标为尚在开发的系统能力，且[客户端授权必须向 Apple 申请](https://developer.apple.com/documentation/BundleResources/Entitlements/com.apple.developer.endpoint-security.client)。它不能直接作为当前 CI 所覆盖 macOS 版本的 pip-only 默认方案，现阶段只列为后续架构研究方向，未改变 R2.2 门禁。
- 工作台启动信息原先只看是否传入 `--enable-autonomous` 就打印“自主执行：已启用”，而策略合同未就绪时实际会在工单开始前阻断。现在区分“未启用”“已启用”和“已配置，执行前阻断（策略级隔离未就绪）”；此修正只调整面向使用者的状态口径，不放宽任何执行门禁。
...
- macOS `doctor` 新增仅供诊断的同组清理局部探测：在 Seatbelt 策略下分别模拟主进程正常退出和命令超时，要求子进程先写出启动标记、broker 组级清理成功、延迟残留标记未出现；异常或缺少 `sandbox-exec` 按失败报告。双架构 CI 对这条探测设置独立测试入口。它不改变 10 项合同 `executed=false` 或自动模式门禁。[修订后 CI](https://github.com/ayukyo/icode/actions/runs/35977398815) 的 macOS Intel/Apple Silicon 原生作业及整套作业均通过，只证明所列局部探测。
...
- R2.3 Windows Job 的第一块为独立、尚未接生产自动链的 `run_windows_job`：以 `CreateProcessW(CREATE_SUSPENDED)` 启动，先加入专属 `KILL_ON_JOB_CLOSE` + 活动进程数上限的 Job，再恢复执行；正常退出与超时均终止 Job 并查询活动进程数。CI 在 Windows x64/arm64 实测另建进程组的后代不能延迟写出标记。该原语**没有**受限身份、ACL、WFP、UAC 或输出代理，不得将 Job 清理探测解释为 Windows 沙箱已就绪；双架构 CI 结果待确认。
