...
- macOS 清理的下一候选是系统自带 `launchd` 独立作业：Apple [launchctl/launchd 文档](https://github.com/apple-oss-distributions/launchd/blob/main/man/launchctl.1)提供按唯一 label 提交/移除作业，[其他开源 Agent 的现场问题](https://github.com/NousResearch/hermes-agent/issues/69098)提示仅 `setsid` 不一定能逃离 launchd 作业的更宽生命周期。当前只增加双架构 CI 可行性探针：唯一临时 label、精确移除、短寿命脱组孙进程；**尚未证明可行，尚未接入生产 broker**。若不能真正回收脱组后代，保留自动模式阻断。
