...
- `read_file` 原先由宿主再次按路径跟随链接打开，直接调用策略工具可读取策略读根之外的链接目标；负例已复现。现解析目标后复核策略读根/拒读根，并复用目录 fd 无跟随打开；指向工作区内文件的链接、既有分段/UTF-8 行展示及普通会话显式外部文件读取均有兼容测试。普通会话中守卫批准与工具解析之间的外部目标替换、Windows 并发重解析和写工具打开竞态尚不能以这些测试证明消除；自动模式保持阻断。
