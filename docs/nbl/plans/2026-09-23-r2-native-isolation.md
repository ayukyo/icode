...
- 自动执行门禁现要求后端明确声明 `policy_contract_ready`，且仍逐步调用 `prepare_policy`；仅有 `wrap_policy`/`prepare_policy` 方法的实验性 Linux 助手不再可能被误当成完整合同后端，CLI 的“已强制隔离”展示也使用同一判断。Landlock 显式声明 false，受控联测入口仍可直接测试其已实现的子集；完整进程数、清理及 Git/网络代理等合同未验收前不得把此标志改为 true。无 `WorkspaceSession` 的程序化 executor 路径仍需单独收紧，工作台生产入口已创建该会话但不能据此忽略公开类的误用风险。
