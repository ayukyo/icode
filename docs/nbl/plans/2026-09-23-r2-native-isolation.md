...
- [首轮 macOS 双架构 CI](https://github.com/ayukyo/icode/actions/runs/35976734089) 的新增探测均在 0.1 秒内报告子进程启动标记缺失，既有 Seatbelt + broker 文件/网络联测仍通过。复查发现探针新脚本误用 `subprocess.DEVNULL`，而实验 profile 未授权打开 `/dev/null`，与启动失败现象一致；本轮不能据此判定组清理失败或通过。已改为将子进程输出写入任务临时工作区，并增加无沙箱编排阳性测试与不泄露路径/命令的分类诊断；等待两架构重跑确认。
