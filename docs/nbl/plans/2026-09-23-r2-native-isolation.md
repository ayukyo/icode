...
- macOS 清理的候选是系统自带 `launchd` 独立作业：Apple [launchctl/launchd 文档](https://github.com/apple-oss-distributions/launchd/blob/main/man/launchctl.1)提供按唯一 label 管理作业，[其他开源 Agent 的现场问题](https://github.com/NousResearch/hermes-agent/issues/69098)提示仅 `setsid` 不一定能逃离 launchd 作业的更宽生命周期。[首轮双架构可行性探针](https://github.com/ayukyo/icode/actions/runs/35961377590)否定了简单的 `submit/remove`：脱组孙进程仍延迟写出残留。现改为仅验证当前用户域的临时 LaunchAgent `bootstrap/bootout`，使用随机唯一 label 和精确 service target，绝不操作整个用户域；**尚未证明可行，尚未接入生产 broker**。若不能真正回收脱组后代，保留自动模式阻断。
