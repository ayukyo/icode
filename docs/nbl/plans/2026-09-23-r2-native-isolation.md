...
- `grep` 同链审计：默认目录扫描原先递归找到链接文件后以 `read_text()` 跟随读取，虽然守卫批准的是起始目录，外部正文仍能回到模型；失败测试已复现。递归扫描现复用无跟随枚举，逐个普通文件由工作区目录 fd 锚定打开，拒读目录不进入结果；显式外部目录仍依赖上层守卫单独批准，工具不把它混入默认扫描。Windows 普通会话仅作链接/接合点和解析路径检查，自动模式仍阻断，因此不宣称其并发重解析已解决。`read_file` / 写工具的普通会话打开竞态仍是后续核查项。
