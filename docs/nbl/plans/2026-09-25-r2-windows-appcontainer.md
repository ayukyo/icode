...
- **采纳 / 暂缓：**保留一次 UAC setup + 非管理员普通任务的目标；不把 Codex 代码结构当作权限可用性证明，也不预先给 sandbox 用户新增敏感 privilege。下一切片先由 x64/ARM64 CI 建立可自动销毁的临时标准账户，真实尝试 `CreateProcessWithLogonW → restricted token → CreateProcessAsUserW`，通过 token API 验证 runner / 子进程不是管理员、子进程 token 确实受限且 SID 不变。日志只记录脱敏的布尔结果与 Win32 错误码，不输出 SID、用户名、组名或权限清单。任何失败都必须在命令启动前报告，并立即阻断该路线；不得退回当前用户裸 `CreateProcess`。
