...
- 日期：2026-09-27
- 当前 R2/R3 状态（2026-09-27 UTC）：R3 有界修复真实闭环已在受控 MiniMax-M3 靶场通过；`base_commit_sha`、初始/受测工作区指纹、稳定受测原始 Git tree 投影 OID 与可选结果 commit tree 只读比对已进入验证证据。仅在显式提供 `--result-commit` 时执行比对；相等只代表 tree 内容匹配，不认证签名、来源或创建时序。13 项定向回归通过，全仓 preflight 三道守护通过（883 项通过、23 项跳过）。受测 tree OID 仅在测试前后稳定时签发；Python SHA-1 OID 不作为签名/抗碰撞认证，commit 对象读取上限 1 MiB；Windows、Git attributes clean 转换、嵌套 `.git`/子模块与宿主环境不在此投影内。workflow Reviewer 跨平台 OS 只读边界与终结器真模型证据仍未闭合。本代码切片 push 后 CI 尚待运行；此前主分支常规 [CI #214](https://github.com/ayukyo/icode/actions/runs/36254293856) 成功，但 Linux/macOS 原生隔离均 `5/10`、`critical_passed=false`、`ready=false`，Windows 标准用户管道探针跳过。前次手动 [CI #213](https://github.com/ayukyo/icode/actions/runs/36252440492) x64/ARM64 标准用户管道均在 `CreateFileW` 处以 `winerror=5` 失败。R2.4 Git broker 仅为 Linux x86_64 内部原型，模型入口继续 `git_broker_unavailable`。R2、R3 继续进行，自动模式和 Git 工具入口保持关闭。
...
- 状态：**R2.1 工作区边界已验收；R2.2 六个 Linux/macOS runner 的原生负例矩阵、R2.3 Windows AppContainer 退出生产候选、R2.4 Linux 内部 Git broker 的边界详见上文与各闸门记录；Windows 自动模式、Git 工具入口和网络均保持关闭。R3 自验证/有界修复及受控 MiniMax-M3 闭环已有离线证据；`VerificationEvidence` 将真实基线 `base_commit_sha`、初始/受测工作区快照指纹、稳定原始 Git tree 投影 OID、可选结果 commit tree 比对、改动 `diff_fingerprint`、产物哈希和独立测试结果共同纳入验证指纹/回执，Reviewer 结束时复核受测 tree。13 项本轮定向回归通过，全量 preflight 为 883 项通过、23 项跳过。此前主分支常规 CI #214 成功但 Linux/macOS conformance 仍 `5/10`、critical/ready 均 false；Windows 标准用户探针跳过，前次手动 CI #213 双架构管道仍以 `CreateFileW winerror=5` 失败。本轮 tree 匹配仅比较路径/字节/模式内容，不代表 clean-filter 结果、宿主属性、环境、签名或安全认证；push 后 CI 尚待运行。Reviewer 终结器真模型证据和工作流 Reviewer 跨平台只读 OS 边界仍未验收。**
...
本地 `VerificationEvidence` 使用真实 `base_commit_sha`、初始/受测工作区快照指纹与稳定受测 Git 原始 tree 投影 OID，并保留本轮 `diff_fingerprint` 与产物哈希；这些字段进入证据指纹/回执/Reviewer 输入，Reviewer 结束时重新核对受测工作区/tree。非 Git 靶场的 SHA 留空；无 Git 可执行文件时非 Git 工作区保持可用，检测到 Git 元数据却无法取 SHA 则失败关闭；基线 SHA 不冒充结果 commit。43 项 R3 回归和全量 preflight（871 项、23 skipped）通过。受测 tree 当前仅支持 POSIX 仓库根，纳入 ignored/未跟踪原始文件，不执行 attributes/filter；资源上限 250,000 项、128 层、256 MiB。Python SHA-1 OID 仅作兼容性内容标识，不能单独作为安全认证；结果 commit `^{tree}` 比较与 receipt/reviewer 最终绑定仍未实现。计划与边界见 [R3 自验证与有界修复](./nbl/plans/2026-09-26-r3-self-verification.md)。

**R3 仍未验收的部分（如实标注）**：工作流 `step=review` 的 Reviewer 命令及策略化命令继续 fail-closed，跨平台原生只读边界仍需 CI 复验；Reviewer 短上下文终结器缺少真模型路径证据。稳定受测 Git 原始 tree 投影 OID 已进入本地证据，但尚未与结果 commit `^{tree}` 精确比较，也未完成最终 receipt/reviewer 结果绑定。专项范围包括 staged/unstaged、ignored/untracked、可执行位、符号链接、attributes/clean filter 与测试期间漂移；当前明确把 ignored 项纳入 raw projection、拒绝嵌套 `.git`/特殊文件/竞态，attributes 不执行，因此不可把 OID 解释为 Git clean-filter 结果。投影不证明 ACL、xattr、所有权、运行环境或进程执行轨迹；SHA-1 标识不作安全认证。基线 SHA 与受测 tree 锚点不等于 R3 全部退出条件通过。
