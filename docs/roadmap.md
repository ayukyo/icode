...
R2.2 已合入 main 持续验证 Linux/macOS 原生后端。[CI #85](https://github.com/ayukyo/icode/actions/runs/36001610973) 的 Ubuntu 22.04/24.04 x64/ARM64 原生与 wheel 测试全部通过，只验收 Linux PID 清理子项；两平台完整退出条件尚未通过，自动模式仍阻断。R2.3 当前开发 Windows AppContainer + Job Object 的原生实验；CI #91–#93 的 Windows x64/ARM64 原生测试均失败，#92/#93 在 CreateProcessW 返回 203。补齐 Windows 自定义环境块盘符项后错误不变，新增空环境块 + System32 程序诊断以区分问题来源；此实验不接自动工单。默认无网络能力的 AppContainer 是候选实现，域名代理仍需独立网络强制机制。R2.4 继续实现网络控制与 Git broker；R2.5 负责安装引导和发布矩阵。开发期线上只保留 main，不以合入主线代替阶段验收。
...
R2.3 Windows AppContainer 原生探针现已有 CI #91–#93 结果，Windows x64 与 ARM64 均失败；#92/#93 的可见诊断是 CreateProcessW 错误码 203。自定义环境块的盘符当前目录项已按 Microsoft 文档修正但未改变结果；当前加空环境块与系统程序对照诊断。AppContainer 仍为实验态，不接自动工单。
