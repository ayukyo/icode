...
- 日期：2026-09-26
- 最新 Windows 诊断（2026-09-26 UTC）：手动 [CI #206](https://github.com/ayukyo/icode/actions/runs/36243032179) 在 Windows x64 与 ARM64 均得到 `dacl_present+ace_match+token_child_process+logon_enabled+restricted_no+access_allow`，但实际客户端 `CreateFileW` 仍为 access denied。下一轮仅观察真实 pipe 对象的 `LABEL_SECURITY_INFORMATION` mandatory-label ACE/`NO_WRITE_UP` bit，以及目标 child token 的 `TokenIntegrityLevel`/`TokenMandatoryPolicy`；只输出固定类别，不记录 SID/RID。微软文档确认 MIC 在 DACL 之外先行判定，完整 SACL 不需要也不会读取；观察到 IL/policy 相符也仍只是根因候选，不能替代内核调用结果。原生双架构复验待新提交 CI；Windows 自动模式保持关闭。
- 最新 Windows 诊断（2026-09-26 UTC）：手动 [CI #204](https://github.com/ayukyo/icode/actions/runs/36242591553) 的 x64/ARM64 诊断标签一致：`dacl_present+ace_match+token_child_process+logon_enabled+restricted_no+access_unavailable`，而 `CreateFileW` 仍为 access denied。对照 Microsoft API 合同发现诊断只请求 DACL 的部分安全描述符，`AccessCheck` 需要 owner/group SID；已按 TDD 加入 owner+group+DACL 查询并有回归覆盖。当前只是可验证根因候选，修正后双架构结果未出；不据此放宽 ACL 或宣布通过。
- 最新诊断补充（2026-09-26 UTC）：手动 Windows [CI #202](https://github.com/ayukyo/icode/actions/runs/36242053362) 在 x64/ARM64 均仍报告通用 `RuntimeError`；复核源代码发现子进程诊断以 `+` 连接的固定标签被外层异常安全过滤器拒绝，导致原始权限诊断被遮蔽。已新增有界标签白名单和负例，保留合法的 DACL/token 标签，同时拒绝路径或任意异常正文；定向探针 31 项及全量 preflight 通过。该修正尚待新的双架构原生 workflow_dispatch；R2/R3 仍未完成。
- 最新状态补充（2026-09-26 UTC；覆盖本文中较早的 R2/R3 快照）：手动 Windows CI [#200](https://github.com/ayukyo/icode/actions/runs/36240714328) 已确认标准用户探针 x64/ARM64 均在子进程 `CreateFileW` 打开管道时 `access_denied`；其他 Windows Job/wheel/workspace 作业通过，Linux/macOS conformance 仍为 `5/10` 且未 ready。当前本地改动加入仅在该失败阶段触发的只读诊断：以真实子进程 token 对照服务端管道 DACL，不改变 ACL 或访问掩码；Windows 代码路径尚待新 CI 原生验证。R3 真模型修复闭环仅在受控 MiniMax 靶场通过；工作流 Reviewer 跨平台边界与真实 Git SHA 锚点仍未闭合。因此 R2、R3 均继续进行，不宣称完成。
- 最新状态补充（2026-09-26 UTC）：工作流 `step=review` 已接入 `_run_agent` 独立只读上下文：源码写工具拒绝，审查产物仅经宿主 `ArtifactBroker` 提交；因为 OS wrappers 不能证明命令看不到 `.icode_output` / 实际嵌套 `out_dir`，`run_command` 继续 fail-closed，策略化 Review 命令亦拒绝。任务级 `run_task` 的 Reviewer 精确读取改动文件、通过 `submit_review` Schema 工具提交；若完成只读取证后在长上下文漏提交，新增只接收完整读取源码（总量最多 64 KiB）、仅开放具名提交工具的短上下文终结器，预算、schema、finding/evidence/diff 校验及失败关闭不变。MiniMax-M3 TDD 靶场已完成一次真实失败分类 → `repair_decisions=["allow"]` → 修复 → 独立 20 项测试通过 → Reviewer 合法提交的闭环（25 次调用 / 109,087 tokens）；单次 17 项测试任务也通过（14 次调用 / 64,427 tokens），但短上下文兜底目前只有 FakeBackend 证据。上次全量 `unittest` 830 项通过（23 skipped），本轮新增 Windows 回执分类后预检正在复跑。手动 Windows [CI #198](https://github.com/ayukyo/icode/actions/runs/36239940582) x64/ARM64 均由新桥接得到子进程回执，但标签仍 `unclassified`；本地刚修正大小写映射，待下一次双架构复验。真实 Git SHA 锚点、工作流 Reviewer 跨平台只读边界和 R2 Windows pipe 原生通过仍未闭合。
...
- 状态：**R2.1 工作区边界已验收；R2.2 六个 Linux/macOS runner 的原生负例矩阵在 #158、#171 均通过，十项合同评分已接线（`conformance_evidence` + doctor + 原生探针 CI），但完整十项在 doctor 上保守为「仅直接证据通过」，仍不等于平台验收闭合。CI #188 暴露的 macOS 同组清理证据与 Windows 局部自检混淆，已由 [CI #189](https://github.com/ayukyo/icode/actions/runs/36223730362) 在 Linux 多架构、macOS Intel/Apple Silicon、Windows x64/ARM64 和 Python 3.11/3.12 全部复验通过；[CI #190](https://github.com/ayukyo/icode/actions/runs/36225072320) 又验证后置 unittest 继承执行器 sandbox，所有常规平台 job 通过。标准用户受限 token 手动探针在 push 触发中跳过，仍需单独复跑。R2.3 Windows AppContainer 综合步骤在 #158 仍失败，已退出生产候选；恢复的一次 UAC native helper 路线仍未实现。Windows 标准用户受限 token 正向路径及错误账户/密码失败关闭已在 #171 x64/ARM64 通过；runner IPC v1 已有 Python 协议与具名管道 primitives，但 #184 的标准用户自连负例在 `CreateFileW` access denied，手动双架构诊断待复验；以上不代表生产 helper、UAC setup、文件/网络 ACL 或 WFP 完成。R2.4 Linux Git broker 仍为未接入工具链的内部原型；Windows 自动模式、Git 工具入口和网络均保持关闭。R3 自验证与有界修复核心切片已实现并离线验收：失败分类 / 证据绑定 / 有界修复决策 / runner 补救回合证据门 / 独立 Reviewer（只读上下文 + 证据引用）已接线到 `run_task`、回归证据绑定到具体 diff（`diff_fingerprint`）并写入事件链与证据包；`run_task` 具备有界修复循环（失败 → 分类 → 有界修复 → 回归 → 独立 Reviewer，`max_repairs` 有界、无新证据即停止），且后置独立测试使用与执行器相同的 sandbox；本机 Linux bwrap 对工作区外哨兵与指定 key 路径均拒绝打开，且仅只读挂载 Python 运行时前缀。Windows 仍没有满足合同的文件/网络隔离。R3 真模型修复分支已被触发但因执行器 max turns 与 Reviewer JSON 合同失败而 fail-closed；完整退出门槛（端到端真模型修复成功、review 步骤 Reviewer 上下文、commit Git SHA 锚定）未闭合。**
...
R3 回归切片已合入 main（2026-09-26，同批）：`workspace_snapshot.diff_fingerprint` 将改动绑定为确定性指纹并进入 `VerificationEvidence`；工作区快照排除 `__pycache__`；`control.record_verification` 幂等写入补救证据事件链并随 `verifications.json` 取证；`run_task` 有界执行失败分类、修复、独立回归，要求出现新失败证据才重试。当前工作区又接入真正独立模型 Reviewer：无写/命令权限，Guard 精确允许读取本次改动文件；报告 schema、changed-file 定位、行号、证据引用及审查前后 diff 锚点均经失败关闭检查。MiniMax-M3 真模型单次代码改动 → 12 项独立测试 → 语义 Reviewer 已在本机 bubblewrap 靶场通过；没有失败、因此不证明自动修复重试。Reviewer 聚焦测试通过，但全量测试和跨平台 CI 仍待回填；本机实现不等于跨平台验收。

**R3 仍未验收的部分（如实标注）**：真模型完整有界修复循环（真实失败 → 分类 → 至少一次真实修复 → 回归 → 独立 Reviewer）尚未实测；工作流 `step=review` 的 Reviewer 命令及策略化命令继续 fail-closed，跨平台原生只读边界仍需 CI 复验；验证证据锚定真实 commit SHA 需待 R2.4 Git broker 接通。R3 完整退出门槛（架构 §13.7）未闭合。
