...
- 日期：2026-09-26
- 最新 Windows 诊断（2026-09-26 UTC）：手动 [CI #208](https://github.com/ayukyo/icode/actions/runs/36244885627) 在 x64 与 ARM64 得到相同观测：`dacl_present+ace_match+token_child_process+logon_enabled+restricted_no+access_allow+client_il_medium+pipe_il_absent+pipe_nwu_unavailable+token_nwu_yes`，但真实客户端 `CreateFileW` 仍 `access_denied`。这轮只查询目标进程 primary token；pipe 无显式 mandatory-label ACE，微软默认语义按 medium 处理，当前证据不支持“低 IL 写高 IL pipe”的 MIC 假设。后续同线程 effective-token 只读诊断已本地实现：私有 CI 入口在 `WaitNamedPipeW` 成功后、每次 `CreateFileW` 前读取线程 token；仅 `ERROR_NO_TOKEN` 时回退 primary token，并记录 IL、两个已定义 mandatory-policy 位、受限状态、logon SID 状态和该次 Win32 错误码。公开 pipe-open API 未增加任意回调，实际观察闭包只查询、不改 token；请求权限/flags、DACL/SACL 均未改。Linux 定向单测通过，但 Windows x64/ARM64 原生复验待新提交，且诊断增加的采样延迟可能改变短暂 pipe 实例状态，不能视为根因已定位。Windows 自动模式保持关闭。Linux/macOS conformance 仍 `5/10`、`critical_passed=false`、`ready=false`。
