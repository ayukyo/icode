...
本文是产品级总架构，不直接作为一个跨 R0–R7 的单体实施计划。每个阶段必须在启动前形成独立规格、影响面和验收计划；前一阶段通过退出门槛后，才展开下一阶段。当前仓库已经具备 R0 基线和 R1 工作台/自动运行基础，用户已决定启动 R2 设计；R1 的完整六阶段非玩具任务验收仍作为独立发布门槛保留，不能因 R2 开始设计而自动视为通过。
...
正式设计见 [R2 跨平台隔离执行环境设计](./nbl/specs/2026-09-23-r2-cross-platform-isolation-design.md)。该规格统一 Linux、macOS、Windows 的用户可见能力，安装入口仅依赖 Python 包。官网按最终产品能力和用户效果展示；README、路线图、发布说明与版本标记仍必须准确区分设计目标和已经通过的验收结果。
