...
- 注：2026-09-25 补充 R2.3 Windows 属性列表差分与 R2.4 Git porcelain v2 解析器研究；20 项观察名单最近全量复核为 2026-09-24。
...
| R2 Windows 默认断网 | [Microsoft AppContainer](https://learn.microsoft.com/en-us/windows/win32/secauthz/implementing-an-appcontainer)、[Codex restricted token](https://github.com/openai/codex/blob/3e9d1d29370ee7239585b9d1d576bea8263768ec/codex-rs/windows-sandbox-rs/src/token.rs)、[Qwen Windows 前置](https://github.com/QwenLM/qwen-code/blob/330b92811c07483e30704190c7e135161120481b/docs/users/features/sandbox.md)、[Gemini Windows ACL](https://github.com/google-gemini/gemini-cli/blob/87de0b6369f0466da37d9b3c0c9b77374bb59992/docs/cli/sandbox.md)、[MXC DACL 性能报告](https://github.com/microsoft/mxc/issues/572)、[Codex AppContainer 路径报告](https://github.com/openai/codex/issues/45871) | **AppContainer 保持候选，不予接入**：当前实验按每次命令扫描并递归加/撤工作区 ACL，尚未证明大仓性能，也未证明 pip 安装的 Python 运行时/ICODE 本身能在容器内启动；MXC issue #572 报告相似路径的全树 ACL 传播可很慢。Codex issue #45871 是单一用户报告，提示 `Path.resolve` 类路径规范化风险；ICODE 源码多处实际使用 `Path.resolve`，必须用原生测试验证。 | CI #91–#99 的 Windows x64、ARM64 AppContainer 原生测试均失败；#92–#99 可见 `CreateProcessW` 错误码 203，#96–#99 普通 Job 空环境对照双架构通过。问题收窄到 AppContainer 启动路径，但 hosted runner 根因未证实，因此没有 Windows 文件/网络隔离验收结论。若后续启动探针通过，仍需大目录基准、IPv4/IPv6/UDP/DNS 外联负例、Git/凭据边界、profile/ACL 多轮清理，再决定是否重设一次 per-workspace-session ACL 生命周期。WFP/代理继续独立门禁。 |
...
| R2 Git/网络 broker | [Codex Linux 沙箱](https://github.com/openai/codex/blob/61e23bc6a1e9e5fc8ed6aefb3575c9d1fd077c22/codex-rs/linux-sandbox/README.md)、[内部 Git 查询](https://github.com/openai/codex/blob/61e23bc6a1e9e5fc8ed6aefb3575c9d1fd077c22/codex-rs/git-utils/src/info.rs)、[应用网络策略](https://github.com/openai/codex/blob/61e23bc6a1e9e5fc8ed6aefb3575c9d1fd077c22/codex-rs/app-server/README.md#application-network-policy)、[Qwen 沙箱](https://github.com/QwenLM/qwen-code/blob/d33cd4ddcb5ce4c8f98df42210ca967707b3b9cd/docs/users/features/sandbox.md)、[Git porcelain v2](https://git-scm.com/docs/git-status) | **采纳原则，实施待验收**：新增仅解析 NUL 分隔 porcelain v2 字节流的内部模块；不启动 Git、不接工具、不建立安全边界。真正代理仍需绑定可信会话、固定参数、OS 级元数据只读和无网络，未移植路径 fail-closed。参见[R2.4 门禁](./nbl/plans/2026-09-24-r2-git-broker-gate.md)。 | `git_broker_unavailable` 保留至恶意仓库扩展、原仓无写、路径注入等负例通过；parser 独立覆盖任意路径字节、重命名双路径和损坏流；raw IP/重定向/UDS/代理失效及凭据隔离属网络代理独立验收；三平台 wheel 实测前不升级能力。 |
| R2 临时网络授权 | [Codex Linux 桥接](https://github.com/openai/codex/blob/282cd7b019378746cb87bd91a95d8b4bcae12aa3/codex-rs/linux-sandbox/README.md)、[Gemini 严格代理 profile](https://github.com/google-gemini/gemini-cli/blob/87de0b6369f0466da37d9b3c0c9b77374bb59992/packages/cli/src/utils/sandbox-macos-strict-proxied.sb)、[Qwen 新模式](https://github.com/QwenLM/qwen-code/blob/11c87ee7c27dbc98efd0f67bb82f19b027f3e610/docs/users/features/sandbox.md) | **选择性采纳、尚未实现**：基础策略继续默认断网，可信宿主独立签发短时 lease；平台 OS 边界只允许到达代理，代理按域名/期限复核并撤销存活隧道。`HTTP_PROXY` 不算保护，Windows Job 不算网络隔离。见[网络门禁](./nbl/plans/2026-09-24-r2-network-proxy-gate.md)。 | raw IP/私网/DNS 重绑定/重定向/直连/UDS、代理失效与到期旧连接、会话串权、凭据隔离，以及三平台干净 wheel 负例未通过前保持 fail-closed。 |
