...
- **上游机制观察：**Codex `rust-v0.155.0-alpha.16` 在 macOS 组信号返回 EPERM 时，可枚举该 PGID 成员，再逐 PID 检查当前 PGID 后发信号；其测试覆盖模拟 EPERM 及 leader 存活/已退出场景。[实现](https://github.com/openai/codex/blob/rust-v0.155.0-alpha.16/codex-rs/utils/pty/src/process_group.rs) · [测试](https://github.com/openai/codex/blob/rust-v0.155.0-alpha.16/codex-rs/utils/pty/src/process_group_tests.rs)。Apple `libproc.h` 明确将该头文件接口说明为 private、未来可能变化；因此不直接引入 `proc_listpgrppids`。[Apple header](https://github.com/apple-oss-distributions/xnu/blob/main/libsyscall/wrappers/libproc/libproc.h)
...
- **#182 结果刷新：**手动 [CI #182](https://github.com/ayukyo/icode/actions/runs/36196277700) x64 与 ARM64 都返回 `client_access_denied+server_accept_timeout`，不再支持“只是 server PID mismatch 后客户端关闭”这一唯一解释；此结果仍未指出 `WaitNamedPipeW`、`CreateFileW` 或 server-PID 查询中的具体拒绝点。
- **来源复核与差异：**固定的 Codex [`runner_pipe.rs`](https://github.com/openai/codex/blob/c7e80f873f67dbef58206b9d4f3c60e9d556eb16/codex-rs/windows-sandbox-rs/src/elevated/runner_pipe.rs) 显示父端先创建 pipe，再通过 `GetNamedPipeClientProcessId` 在服务端校验刚启动的 runner PID；它不构成客户端用 `GetNamedPipeServerProcessId` 查询服务端 PID 的先例。Microsoft [`GetNamedPipeServerProcessId`](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-getnamedpipeserverprocessid) 对句柄的说明写明须由 `CreateNamedPipe` 创建，而 ICODE 当前调用方拿到的是 `CreateFileW` 客户端句柄。记录为待验证 API 契约疑点，不将其写成已证实根因，也不因上游设计省略某个对称检查就直接削弱本地门槛。
- **下一步取舍：**采纳按 Win32 调用点划分的安全诊断：分别标记 pipe wait、client open、server PID query 的拒绝结果，原始异常保持不外传；暂缓改 ACL 或移除身份核验。该阶段的纯单测通过，阶段码还待 #183+ 原生双架构验证；只有明确是哪一 API 被拒，才评估最小 API/调用链修正。Windows 后端、UAC setup、完整文件/网络隔离与自动模式仍未完成。
