...
### 2026-09-26 UTC Windows loader 分层诊断研究与取舍
...
### 2026-09-26 UTC：R2.3 Windows wheel 与 helper 发布链复核
...
### 2026-09-26 UTC：Codex Windows 双路线与 helper 职责边界复核
...
- **成本、许可和验收：**Codex 主仓 Apache-2.0；具体文件/依赖仍需逐项许可核对，ICODE 本轮仅借鉴边界，没有复制代码或新增依赖。尚需 Authenticode 发布者信任、SmartScreen/企业策略、x64/ARM64 原生 helper 以及 UAC 取消/配置恢复/命令未启动的 Windows 实测。观察日期：2026-09-26。
...
### 2026-09-26 UTC：普通用户派生 Windows 受限进程的前置核验
...
- **成本、安全与兼容性：**计划中的探针仅使用 Win32/Windows runner，不复制 Codex/Gemini 源码或增加 ICODE 用户安装依赖；临时 sandbox 凭据只服务于一次性 CI 本地账户并在退出时删除。Codex 路线的明文密码 API、DPAPI 存储与同登录用户进程边界，不能夸大为可抵御同一登录用户权限下的恶意进程。观察日期：2026-09-26。
