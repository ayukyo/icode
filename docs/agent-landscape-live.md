...
- **ICODE 采纳 / 暂缓：**采纳“是否独立验证不构成权限豁免”：agent 命令与会执行仓库代码的自动测试应经过同一 OS sandbox runner；可信宿主侧只读结果，不执行仓库代码。当前 `run_task` 已把同一 sandbox 传给独立 unittest；bwrap 为非系统 Python runtime 只读开放精确安装前缀，不开放其上级 home。Linux 外部哨兵与模型 key 路径的打开拒绝已本机验证，Python 3.11 全量 preflight 与各平台对应 profile/CI 仍待完成。本轮未复制上游代码，无新增依赖或第三方许可义务；尚待三平台同一文件/网络负例矩阵，不将本地 Linux 成功外推。
