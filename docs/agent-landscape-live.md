...
| R2 Windows 默认断网 | [Microsoft AppContainer](https://learn.microsoft.com/en-us/windows/win32/secauthz/implementing-an-appcontainer)、[Codex restricted token](https://github.com/openai/codex/blob/3e9d1d29370ee7239585b9d1d576bea8263768ec/codex-rs/windows-sandbox-rs/src/token.rs)、[Qwen Windows 前置](https://github.com/QwenLM/qwen-code/blob/330b92811c07483e30704190c7e135161120481b/docs/users/features/sandbox.md)、[Gemini Windows ACL](https://github.com/google-gemini/gemini-cli/blob/87de0b6369f0466da37d9b3c0c9b77374bb59992/docs/cli/sandbox.md) | **选择性采纳 AppContainer 候选**：无 `internetClient` / `privateNetworkClientServer` 能力的每工单 Package SID + 限定任务目录 ACL，与 Job Object 进程数/回收组合；纯 `ctypes`，不增加 Python 依赖。Codex restricted token 可借鉴令牌最小化，但不等同文件/凭据隔离；Qwen Docker/Podman 不符合 pip-only；Gemini 低完整性 ACL 可能残留，不直接照搬。 | [R2.3 Windows AppContainer 计划](./nbl/plans/2026-09-25-r2-windows-appcontainer.md) 的临时 ACL 回滚、任务目录读写、外部文件读写拒绝、localhost 拒绝、子进程回收、Job 上限及 Windows x64/ARM64 原生测试；目前仍为实验入口，不接自动模式。需要域名代理授权时另做 WFP/防火墙特权边界，不能用 `HTTP_PROXY` 代替。 |
