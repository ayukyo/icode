...
| Windows sandbox | Codex `3e9d1d2`；Qwen `330b928`；Gemini CLI `87de0b6` | [Microsoft AppContainer 启动](https://learn.microsoft.com/en-us/windows/win32/secauthz/implementing-an-appcontainer) · [隔离模型](https://learn.microsoft.com/en-us/windows/win32/secauthz/appcontainer-isolation) · [Codex restricted token](https://github.com/openai/codex/blob/3e9d1d29370ee7239585b9d1d576bea8263768ec/codex-rs/windows-sandbox-rs/src/token.rs) · [Qwen 沙箱文档](https://github.com/QwenLM/qwen-code/blob/330b92811c07483e30704190c7e135161120481b/docs/users/features/sandbox.md) · [Gemini Windows 沙箱](https://github.com/google-gemini/gemini-cli/blob/87de0b6369f0466da37d9b3c0c9b77374bb59992/docs/cli/sandbox.md) | Windows 机制深读；CI #91–#105 的 Windows AppContainer 启动探针多轮 x64/ARM64 返回 `CreateProcessW` 203；#105 固定 whoami A/B 显示需注入 profile `LOCALAPPDATA` 才能启动。常规路径接入后，#106–#111 的 Python 仍退出 `0xC0000135`；#110 profile marker 两架构均未写入。#112 process-limit 正反对照双架构通过。#138 disposable staging ACL 精确恢复、源 ACL 不变和清理通过；#141 staged Python 已进入脚本/完成导入，但 strict `Path.resolve()` 返回 PermissionError，阻止后续 Python 级边界断言。组件级通过不等于 Windows R2.3 完成，Windows 自动模式继续关闭。 |
