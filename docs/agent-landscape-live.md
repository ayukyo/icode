...
- **下一项只读研究采纳为受限 A/B 诊断，尚无原生结果：**Microsoft AppContainer 样例和固定版本 io-harness 0.86.0 的 `CreateProcessW` 都将 `lpApplicationName` 设为 `NULL`，由可写命令行提供模块路径；ICODE 默认仍显式传绝对 `argv[0]`。已加入仅对无参数的 `SystemRoot\\System32\\whoami.exe` 开放的配对诊断，保留环境、AppContainer、挂起创建与 Job 顺序；外层和底层都拒绝其它目标，用户命令不开放此变体。Windows CI 需同时确认两次实际环境块相等并记录 A/B 结果；在 CI 返回前仍不能判断根因或声称 Windows 达标。[io-harness 固定版本源码](https://docs.rs/io-harness/0.86.0/src/io_harness/sandbox/appcontainer.rs.html#L1439-L1458)、[Microsoft CreateProcessW 参数说明](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessw)。
